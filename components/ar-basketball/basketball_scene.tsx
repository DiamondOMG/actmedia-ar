...
export default function BasketballScene() {
  ...
  // ดึงค่า URL Parameter เมื่อคอมโพเนนต์เริ่มทำงานบนเบราว์เซอร์
  useEffect(() => {
  ...
  // บังคับรีเฟรชหน้าเว็บเมื่อสลับโหมดผ่าน Hamburger Menu เพื่อล้างสถานะ Three.js/XR8 เก่าทั้งหมด
  ...
  // เริ่มต้นสตาร์ทกล้อง WebXR (ทำงานเฉพาะหลังจากโหลดค่าโหมดจาก URL ปลายทางเสร็จสิ้นแล้ว)
  useEffect(() => {
    ...
    <div ref={containerRef} className="absolute inset-0 w-full h-full bg-black overflow-hidden select-none">
        ...
        <div className="absolute inset-0 z-50 flex flex-col items-center justify-center bg-black/80 px-8 text-center text-white backdrop-blur-sm">
          {!gameState.isAssetLoaded ? (
            ...
            <>
              ...
              <p className="text-slate-400 text-sm max-w-xs leading-relaxed mb-6 font-sans">
      ...
      {gameState.isHoopPlaced && (
        <button
          ...
          className="absolute top-4 left-3 z-35 flex h-10 w-10 items-center justify-center rounded-xl bg-black/60 text-white backdrop-blur border border-white/10 hover:bg-black/80 transition active:scale-95 shadow pointer-events-auto"
        >
              ...
              <div className="flex flex-col gap-3 font-sans">
                ...
                >
                  <div>
                    ...
                    <div className="text-[10px] text-slate-400 font-sans">ยิง 3 ระยะ ท้าทายคะแนนตัวคูณ</div>
                  </div>
          ...
          <div className="flex flex-col items-end gap-1.5">
            ...
            {gameMode === "wind" && gameState.windSpeed !== undefined && gameState.windSpeed > 0 && (
              <div className="rounded-xl bg-sky-900/60 backdrop-blur-sm border border-sky-400/20 px-3 py-1.5 text-white font-mono text-xs flex items-center gap-1.5">
                <Wind className="h-3.5 w-3.5 text-sky-300 animate-pulse" />
                <span className="font-bold text-sky-200 font-sans">
      ...
      {showStatus && (
        <div className="absolute inset-0 z-20 flex items-center justify-center pointer-events-none">
          {gameState.status === "scored" ? (
            ...
            <div className="px-6 py-3 rounded-2xl bg-black/85 border border-white/20 text-2xl font-black text-white shadow-2xl animate-bounce font-sans">
              {showStatus}
            </div>
          )}
        </div>
      ...
      {gameState.ballsLeft === 0 && gameState.status === "idle" && (
        <div className="absolute inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-md px-6">
          <div className="bg-slate-900 border border-white/10 rounded-3xl p-7 max-w-sm w-full text-center text-white shadow-2xl animate-in zoom-in-95 duration-200 font-sans">
      ...
      {gameState.isHoopPlaced && gameState.status === "idle" && (
        <div className="absolute inset-0 flex flex-col items-center justify-end pb-32 pointer-events-none z-10">
          <div className="flex flex-col items-center gap-2">
            ...
            <span className="text-white/80 text-xs font-semibold tracking-wider drop-shadow-md font-sans">
      ...
      <div className="absolute bottom-4 left-4 right-4 z-10 flex items-center justify-between pointer-events-auto">
        ...
        <button
          onClick={handleReset}
          className="flex h-12 items-center gap-2 rounded-xl bg-slate-900/80 px-4 text-white backdrop-blur border border-white/10 hover:bg-slate-800 transition font-sans"
        >
